...
async function onClickAcceptContent(interaction, client) {
	if (!interaction.isButton()) return;
	const staff = await interaction.guild.members.fetch(interaction.user.id);
	if (interaction.customId != 'accept_content') return;
		...
		const role = guild.roles.cache.get(config.content_role_id);
